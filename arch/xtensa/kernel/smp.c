...
extern int __init wakeup_secondary_cpu(unsigned int, struct task_struct*);
...
extern void send_ipi_message(const cpumask_t *, int);
...
void smp_send_reschedule(int cpu)
{
	...
	send_ipi_message(&mask, IPI_RESCHEDULE);
}

void smp_send_stop(void)
{
	/* REMIND-FIXME: remove this infinite loop. */
	pr_debug("smp_send_stop()\n");
	while(1);
			      ...
			      int wait, cpumask_t callmask)
{
	...
	if (atomic_read(&data.pending) != 0) {
		pr_debug("%s: Cross Call Timed Out; data.pending:%d; wait:%d\n",
...
irqreturn_t ipi_interrupt(int irq, void *dev_id)
{
	...
	if (msg & (1 << IPI_NMI_DIE)) {
		...
		pr_debug("%s: notify_die(DIE_NMI_IPI, 'nmi_ipi', "
